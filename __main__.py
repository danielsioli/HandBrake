...
class Janela:
    def __init__(self, config_ini, logger_ini):
        ...
        self.armazenar = int(self.config['config']['armazenar'])
    ...
    def iniciar(self):
        ...
        thread_memoria = Thread(target=self.freio_de_mao, args=(self.armazenar, ))
        jogando = True
        try:
            ...
            while True:
                ...
                if event == 'Parar' or event == sg.WINDOW_CLOSED:
                    ...
                    self.config['config']['armazenar'] = str(values[3])
    ...
    def get_layout(self):
        ...
        layout = [
            ...
            [sg.Text('Armazenar (segundos)', size=label_size), sg.InputText(default_text=self.armazenar, size=input_size)],
            [sg.Text('Leitura', size=label_size), sg.ProgressBar(max_value=255, orientation='h', size=(27.25, 25), style='vista', key='freio_de_mao')],
    ...
    def freio_de_mao(self, armazenar=0):
        global jogando
        ...
        if self.devices is None:
            ...
            mouse = Controller()
            while jogando:
                intensidade = self.devices.read(endpoint_address, 1024*2)[5]
                ...
                if not apertado and intensidade >= self.zona_morta:
                    ...
                    apertado = True
                elif apertado and intensidade <= self.zona_morta - 10:
                    teclado.release(self.key)
                    #mouse.scroll(-intensidade, 0)
                    apertado = False
                if armazenar > 0:
                    data = pd.concat([data, pd.DataFrame.from_dict(
                        {'hora': [datetime.today()], 'apertado': [intensidade if apertado else np.NaN],
                        'nao_apertado': [intensidade if not apertado else np.NaN]})], axis=0, ignore_index=True)
                    if data.loc[len(data.index) - 1, 'hora'] > data.loc[0, 'hora'] + timedelta(seconds=armazenar):
                        data.drop(index=0, inplace=True)
                        data.reset_index(drop=True, inplace=True)
            if armazenar > 0 and not data.empty:
                logging.info('Armazenando os dados em data.csv')
                data.to_csv('data.csv', sep=';', decimal=',', encoding='utf-8-sig', index=False)
